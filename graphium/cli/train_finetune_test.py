...
def run_training_finetuning_testing(cfg: DictConfig) -> None:
    ...
    ckpt_path = None
    ckpt_name = cfg['trainer'].pop('test_from_checkpoint', None)
    ckpt_dir = cfg['trainer']['model_checkpoint'].pop('dirpath', None)
    if ckpt_name not in [None, 'none'] and ckpt_dir is not None:
        ckpt_path = os.path.join(ckpt_dir, ckpt_name)
    
    # Run the model testing
    with SafeRun(name="TESTING", raise_error=cfg["constants"]["raise_train_error"], verbose=True):
        trainer.test(model=predictor, datamodule=datamodule, ckpt_path=ckpt_path)  # , ckpt_path=ckpt_path)
