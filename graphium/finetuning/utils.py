...
def modify_cfg_for_finetuning(cfg: Dict[str, Any]):
    ...
    # Quick fix for mup breaking the config
    if cfg_arch['gnn']['layer_type'] == graphium.nn.pyg_layers.mpnn_pyg.MPNNPlusPyg:
        cfg_arch['gnn']['layer_type'] = 'pyg:mpnnplus'

    for module_name, module in cfg_arch.items():
        if isinstance(module, dict):
            print(module_name)
            if 'depth' in module.keys():
                if module['depth'] is None:
                    module['depth'] = len(module['hidden_dims']) + 1
                    module['hidden_dims'] = module['hidden_dims'][0]
                    if 'hidden_dims_edges' in module.keys():
                        module['hidden_dims_edges'] = module['hidden_dims_edges'][0]
            if 'activation' in module.keys():
                print(module['activation'])
                module['activation'] = str(module['activation']).split("(")[0].lower()
                print(module['activation'])
            if 'last_activation' in module.keys():
                print(module['last_activation'])
                module['last_activation'] = str(module['last_activation']).split("(")[0].lower()
                print(module['last_activation'])
            if "layer_type" in module.keys():
                if module["layer_type"] == graphium.nn.pyg_layers.mpnn_pyg.MPNNPlusPyg:
                    module["layer_type"] = "pyg:mpnnplus"
                if module["layer_type"] == graphium.nn.base_layers.FCLayer:
                    module["layer_type"] = "fc"
            for submodule_name, submodule in module.items():
                if isinstance(submodule, dict):
                    print(submodule_name)
                    if 'depth' in submodule.keys():
                        if submodule['depth'] is None:
                            submodule['depth'] = len(submodule['hidden_dims']) + 1
                            submodule['hidden_dims'] = submodule['hidden_dims'][0]
                            if 'hidden_dims_edges' in submodule.keys():
                                submodule['hidden_dims_edges'] = submodule['hidden_dims_edges'][0]
                    if 'activation' in submodule.keys():
                        print(submodule['activation'])
                        submodule['activation'] = str(submodule['activation']).split("(")[0].lower()
                        print(submodule['activation'])
                    if 'last_activation' in submodule.keys():
                        print(submodule['last_activation'])
                        submodule['last_activation'] = str(submodule['last_activation']).split("(")[0].lower()
                        print(submodule['last_activation'])
                    if "layer_type" in submodule.keys():
                        if submodule["layer_type"] == graphium.nn.pyg_layers.mpnn_pyg.MPNNPlusPyg:
                            submodule["layer_type"] = "pyg:mpnnplus"
                        if submodule["layer_type"] == graphium.nn.base_layers.FCLayer:
                            submodule["layer_type"] = "fc"
